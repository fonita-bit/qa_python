...
class TestBooksCollector:
    ...
    # чтобы тесты были независимыми в каждом из них создавай отдельный экземпляр класса BooksCollector()
        ...
        # --2-- --тест на установку книге жанра, только если есть в списке,--
        def test_set_book_genre_add_book_with_genre_and_without(self, new_book):
            collector.add_new_book('Красная шапочка')
            #-- жанра сказка нет
            collector.set_book_genre('Красная шапочка', 'Сказка')
            #-- остается только 2 правильные книги из фикстуры
        ...
        # --6-- --тест возвращает 3 книги, подходящие детям, по одной книге из жанра, им подходят Фантастика Мультфильм Комедии--
        def test_get_books_for_children_add_books_for_children(self):
            collector = BooksCollector()
            collector.add_new_book('Простоквашино')
            collector.set_book_genre('Простоквашино', 'Мультфильм')
